...
const channels = {
  ...
  ch1: {
    input: "http://fackyou-cdn5.cfd/BEIN-1/index.m3u8",
...
async function spawnStream(id) {
  ...
  try {
    ...
    // لو قائمة تشغيل، نجرب الرابط التالي بدل ما نقف تمامًا
    if (sources.length > 1) {
      playlistIndex[id] = (playlistIndex[id] + 1) % sources.length;
      setTimeout(() => {
        if (!ffmpegProcesses[id] && !manuallyStopped[id]) spawnStream(id);
      }, 3000);
    }
    return;
  }
...
"-g", "50",
  ...
  ffmpeg.on("exit", () => {
    ...
    const currentSources = getChannelSources(ch);
...
.formCard input:focus, .formCard textarea:focus{ border-color:var(--accent); }
...
\${hasPlaylist ? '<span class="catTag mono" style="margin-right:6px">🎬 '+(playlistPosition||1)+'/'+(playlistTotal||ch.playlist.length)+'</span>' : ''}
</div>
